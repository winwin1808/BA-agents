export const BA_AGENTS_MCP_SKILL_MARKDOWN = [
  "---",
  "name: use-ba-agents-mcp",
  "description: 'Use when the task needs BA Agents MCP to retrieve trusted BSS B2B Suite context, open canonical documents, check changelog wording, recommend a minimal context bundle, generate a very basic reference UI from a confirmed UX/UI task, or generate a workflow artifact for BA/PM planning. Covers Lock, Quote/Quick, Solution, and cross-suite BA/PM work.'",
  "---",
  "",
  "# Use BA Agents MCP",
  "",
  "## Overview",
  "",
  "Use this skill when the answer should come from the BA Agents MCP instead of memory. The MCP is the trusted retrieval layer for BSS B2B Suite BA/PM context, the entry point for low-fidelity reference UI generation, and the workflow artifact generator for BPMN-style planning output.",
  "",
  "## Preconditions",
  "",
  ...
  "3. Use `prompt` to include the trigger, actors, decision points, exception paths, and end states.",
  "4. Use `context_scope` with one of: `lock`, `quote`, `solution`, or `cross_suite`.",
  "5. Set `include_bpmn_xml` only when the client needs the raw BPMN XML in the tool response.",
  "",
  "## Search Heuristics",
  "",
...
export function buildTokenSharePackage(token: string) {
  return [
    `Authorization: Bearer ${token}`,
    "",
    BA_AGENTS_MCP_SKILL_MARKDOWN,
  ].join("\n");
}
